...
class ConnectionManager:
    ...
    async def _establish_ssh_tunnel(self) -> bool:
        ...
        try:
            ...
            logging.info(
                f"🔧 Creating SSH tunnel: localhost:{self.local_tunnel_port} → {self.ip_address}:15740"
            )
            ...
            if self.ssh_process.poll() is None:
                logging.info(
                    f"✅ SSH tunnel established on port {self.local_tunnel_port}"
                )
    ...
    async def disconnect(self):
        ...
        for name, sock in [
            ("command", self.command_socket),
            ("event", self.event_socket),
        ]:
            if sock:
                try:
                    sock.close()
