...
from .protocol import (
    ...
    create_init_cmd_req_packet,
    create_init_event_req_packet,
...
class SonyCamera(EventHandler):
    ...
    def __init__(
        self, ip_address: str, ssh_username: str = None, ssh_password: str = None
    ):
    ...
    def _handle_operation_response(self, data: bytes):
        ...
        if self.state in [PTP_STATES.OPEN_SESSION, PTP_STATES.OPEN_SESSION + 1]:
            if response_code == PTP_RESPONSES["OK"]:
                ...
                logging.debug(
                    "Session Access_Denied (0x200F) - camera busy, will retry"
                )
            else:
                logging.error(f"❌ Session failed: 0x{response_code:04x}")
                self.emit(
                    "handshakeFailed",
                    Exception(f"Session failed: 0x{response_code:04x}"),
                )
            ...
            try:
                ...
                def send_operation_sync(op_code, params=None):
                    ...
                    packet = create_operation_request(
                        op_code, self.transaction_id, params
                    )
                    self.transaction_id += 1
                ...
                send_operation_sync(
                    PTP_OPERATIONS["SDIO_GET_EXT_DEVICE_INFO"], [0x012C, 0, 0]
                )
    ...
    def _detect_initial_recording_state(self):
        ...
        try:
            packet = create_operation_request(
                PTP_OPERATIONS["SDIO_GET_ALL_EXT_DEVICE_PROP_INFO"], tx_id, []
            )
            self.connection.command_socket.send(packet)
            ...
            while offset < len(data) - 10:
                ...
                if val_sz == 1:
                    ...
                    current_value = struct.unpack(
                        "<H", data[cv_offset : cv_offset + 2]
                    )[0]
                elif val_sz == 4:
                    current_value = struct.unpack(
                        "<I", data[cv_offset : cv_offset + 4]
                    )[0]
                else:
                    current_value = 0
                ...
                state_name = {
                    ...
                    0x03: "Waiting Record",
                }.get(current_value, f"Unknown(0x{current_value:02X})")
            ...
            for evt, handler in [
                ("operationData", on_data),
                ("operationResponse", on_response),
            ]:
            ...
            try:
                ...
                if self.connected:
                    self.keepalive_timer = Timer(
                        self.keepalive_interval, keepalive_task
                    )
    ...
    async def _send_recording_command(self, value: int) -> bool:
        ...
        oper_req, start_data, data_packet, end_data = (
            create_operation_with_data_packets(op_code, tx_id, [prop_code, 0], payload)
        )
    ...
    FOCUS_FRAME_TYPE = {
        0x0001: "PhaseDetection_AFSensor",
        0x0002: "PhaseDetection_ImageSensor",
        0x0003: "Wide",
        0x0004: "Zone",
        0x0005: "CentralEmphasis",
        0x0006: "ContrastFlexibleMain",
        0x0007: "ContrastFlexibleAssist",
        0x0008: "Contrast",
        0x0009: "ContrastUpperHalf",
        0x000A: "ContrastLowerHalf",
        0x000B: "DualAFMain",
        0x000C: "DualAFAssist",
        0x000D: "NonDualAFMain",
        0x000E: "NonDualAFAssist",
        0x000F: "FrameSomewhere",
        0x0010: "Cross",
    }
    FOCUS_FRAME_STATE = {
        0x0001: "NotFocused",
        0x0002: "Focused",
        0x0003: "FocusFrameSelection",
        0x0004: "Moving",
        0x0005: "RangeLimit",
        0x0006: "RegistrationAF",
        0x0007: "Island",
    }
    FACE_FRAME_TYPE = {
        0x0001: "DetectedFace",
        0x0002: "AF_TargetFace",
        0x0003: "PersonalRecognitionFace",
        0x0004: "SmileDetectionFace",
        0x0005: "SelectedFace",
        0x0006: "AF_TargetSelectionFace",
    ...
    def _parse_focal_frame_info(self, ff_data: bytes):
        ...
        try:
            if (
                len(ff_data) < 56
            ):  # minimum: 8 (info) + 40 (reserved) + 8 (array header)
            ...
            version = struct.unpack_from("<H", ff_data, pos)[
                0
            ]  # 100x value (e.g. 100=1.00, 101=1.01)
            ...
            # --- FocusFrame section ---
            frames.extend(
                self._parse_frame_section(
                    ff_data,
                    pos,
                    "focus",
                    self.FOCUS_FRAME_TYPE,
                    self.FOCUS_FRAME_STATE,
            ...
            if version >= 101 and pos < len(ff_data):
                frames.extend(
                    self._parse_frame_section(
                        ...
                        pos,
                        "face",
                        self.FACE_FRAME_TYPE,
                        self.FACE_FRAME_STATE,
            ...
            if version >= 101 and pos < len(ff_data):
                frames.extend(
                    self._parse_frame_section(
                        ...
                        pos,
                        "tracking",
                        self.TRACKING_FRAME_TYPE,
                        self.TRACKING_FRAME_STATE,
        ...
        if frames and not self._focal_frame_logged:
            ...
            logging.debug(
                f"FocalFrameInfo v{version/100:.2f}: {len(frames)} frame(s) — {frames}"
            )
    ...
    def _parse_frame_section(
        self, data, pos, category, type_map, state_map, frame_entry_parser
    ):
        ...
        for i in range(frame_num):
            if entry_offset + 24 > len(data):
                break
            entry = frame_entry_parser(
                data, entry_offset, x_den, y_den, type_map, state_map
            )
            if entry:
    ...
    def _init_liveview(self) -> bool:
        ...
        try:
            ...
            packet = create_operation_request(
                PTP_OPERATIONS["GET_OBJECT_INFO"], tx_id, [LIVEVIEW_HANDLE]
            )
            self.connection.command_socket.send(packet)
            ...
            if error_holder[0]:
                logging.debug(
                    f"GetObjectInfo(liveview) failed: 0x{error_holder[0]:04X}"
                )
                return False

            if result_holder[0] is not None:
                logging.debug(
                    f"Live view initialized (ObjectInfo: {len(result_holder[0])} bytes)"
                )
    ...
    def get_liveview_frame_sync(self) -> Optional[bytes]:
        ...
        try:
            ...
            packet = create_operation_request(
                PTP_OPERATIONS["GET_OBJECT"], tx_id, [LIVEVIEW_HANDLE]
            )
            self.connection.command_socket.send(packet)
            ...
            if error_holder[0]:
                ...
                if err == 0x200F:
                    ...
                    logging.debug(
                        "Liveview not available (0x2009 - camera may not be in shooting mode)"
                    )
                    self._liveview_initialized = False
                else:
            ...
            if img_offset + img_size <= len(data):
                ...
                if len(jpeg_data) > 2 and jpeg_data[0:2] == b"\xff\xd8":
                    ...
                    logging.debug(
                        f"LiveView data at offset {img_offset} is not JPEG "
                ...
                if soi >= 0:
                    return data[soi:]
                logging.debug(
                    f"LiveView Dataset: offset={img_offset} size={img_size} "
    ...
    def set_device_property_sync(
        self, prop_code: int, value: int, timeout: float = 2.0
    ) -> bool:
    ...
    def get_all_properties_sync(self, timeout: float = 3.0) -> dict:
        ...
        default_result = {
            ...
            "iso": empty(),
            "shutter_speed": empty(),
            "exposure_program": empty(),
            "shutter_mode": empty(),
            "shutter_angle": empty(),
            "touch_function": empty(),
            "remote_touch_enable": empty(),
            "remote_touch_cancel_enable": empty(),
            "zoom_setting": empty(),
            "zoom_type_status": empty(),
            "media_slot1_rec_time": empty(),
            "media_slot2_rec_time": empty(),
            ...
            for prop_code, name in want_codes.items():
                ...
                if form_flag == 0x02:  # Enumeration
                    ...
                    if enum_offset + 2 <= len(data):
                        num_vals = struct.unpack(
                            "<H", data[enum_offset : enum_offset + 2]
                        )[0]
                        enum_offset += 2
                        if num_vals <= 200:
                                ...
                                if enum_offset + val_size <= len(data):
                                    if val_size == 1:
                                        ...
                                        allowed.append(
                                            struct.unpack(
                                                "<H",
                                                data[enum_offset : enum_offset + 2],
                                            )[0]
                                        )
                                    elif val_size == 4:
                                        allowed.append(
                                            struct.unpack(
                                                "<I",
                                                data[enum_offset : enum_offset + 4],
                                            )[0]
                                        )
                ...
                if prop_code == SONY_PROPERTIES["F_NUMBER"]:
                    ...
                    SONY_PROPERTIES["MEDIA_SLOT1_REC_TIME"],
                    ...
                    display = self._ENUM_TABLES[prop_code].get(
                        display_value,
                        self._ENUM_TABLES[prop_code].get(
                            current_value, f"0x{display_value:04X}"
                        ),
                    )
                else:
                    display = (
                ...
                logging.debug(
                    f"  Found 0x{prop_code:04X} ({name}): val={current_value} "
    ...
    async def _send_operation_and_wait(
        self, op_code: int, params: bytes = None, timeout: float = 2.0
    ):
        ...
        try:
                ...
                try:
                    ...
                    if data:
                        ...
                        try:
                            ...
                            logging.error(
                                f"🔧 Failed to parse packet header: {parse_error}"
                            )
                        ...
                        if pkt_type == PTPIP_OPER_RESP:
                            ...
                            logging.info(
                                f"🔧 Got START_DATA packet - collecting all data..."
                            )
                            ...
                            while (
                                ...
                                and len(all_data) < 1000
                            ):
                                try:
                                    more_data = self.connection.command_socket.recv(
                                        4096
                                    )
                                    if more_data:
                                        response_count += 1
                                        ...
                                        try:
                                            more_length, more_pkt_type = (
                                                parse_packet_header(more_data)
                                            )
                                            ...
                                            if (
                                                len(more_payload) > 100
                                            ):  # Only collect substantial data
                                    ...
                                    logging.debug(
                                        f"🔧 Error collecting additional data: {e}"
                                    )
                                    break
            ...
            logging.info(
                f"🔧 Timeout after {timeout}s, received {response_count} responses"
            )
            return None
                ...
                try:
                    ...
                    for offset in range(
                        ...
                        if response_data[offset : offset + 2] == zoom_property_bytes:
                            ...
                            logging.debug(
                                f"Found zoom property at offset {offset} but no valid data"
                            )
